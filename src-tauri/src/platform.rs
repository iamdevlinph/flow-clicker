...
fn resolve_point(action: &ClickRef<'_>, origin: Option<(i32, i32)>) -> (i32, i32) {
    match (origin, action.relative_x, action.relative_y) {
        (Some((left, top)), Some(x), Some(y)) => (left + x, top + y),
        _ => (action.screen_x, action.screen_y),
    }
}
...
mod imp {
    ...
    pub fn resolve(action: &ClickRef<'_>, focus: bool) -> (i32, i32) {
        if let (Some(title), Some(_), Some(_)) =
            (action.window_title, action.relative_x, action.relative_y)
        {
            ...
            if !hwnd.is_null() {
                ...
                if let Some(r) = rect(hwnd) {
                    return resolve_point(action, Some((r.left, r.top)));
                }
            }
        }
        resolve_point(action, None)
    }
...
#[cfg(test)]
mod tests {
    use super::resolve_point;
    use crate::models::ClickRef;

    fn click(relative_x: Option<i32>, relative_y: Option<i32>) -> ClickRef<'static> {
        ClickRef {
            id: "click",
            screen_x: 10,
            screen_y: 20,
            relative_x,
            relative_y,
            window_title: Some("Target"),
        }
    }

    #[test]
    fn resolves_relative_points_and_falls_back_to_screen_coordinates() {
        assert_eq!(
            resolve_point(&click(Some(5), Some(7)), Some((100, 200))),
            (105, 207)
        );
        assert_eq!(resolve_point(&click(Some(5), Some(7)), None), (10, 20));
        assert_eq!(
            resolve_point(&click(None, Some(7)), Some((100, 200))),
            (10, 20)
        );
    }
}
