	...
	return actions
		.map((action, index) => {
			...
			const targetMarkup = click
				...
				: "";
			return `<article class="action-card action-${escapeHtml(action.type)}${click && highlighted.has(action.id) ? " action-selected" : ""}" data-id="${escapeHtml(action.id)}"><details class="action-card-accordion"><summary class="action-card-summary" aria-label="Details for ${actionLabel}"><span class="action-card-summary-content"><span class="action-number">${index + 1}</span><span class="action-type ${escapeHtml(action.type)}">${escapeHtml(action.type)}</span><label class="action-name-field"><input class="compact-input action-name" aria-label="Action name" value="${escapeHtml(action.name)}" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false"></label><span class="action-card-chevron" aria-hidden="true">▸</span></span></summary><div class="action-card-body"><div class="action-card-details">${details}</div>${targetMarkup}<div class="action-card-foot"><div class="row-action-group"><button class="icon-btn row-action" data-action="move-up" type="button" title="Move up" aria-label="Move ${actionLabel} up"${index === 0 ? " disabled" : ""}>↑</button><button class="icon-btn row-action" data-action="move-down" type="button" title="Move down" aria-label="Move ${actionLabel} down"${index === actions.length - 1 ? " disabled" : ""}>↓</button><button class="icon-btn row-action" data-action="duplicate" type="button" title="Duplicate action" aria-label="Duplicate ${actionLabel}">⧉</button><button class="icon-btn row-action danger-ghost" data-action="delete" type="button" title="Delete action" aria-label="Delete ${actionLabel}">×</button></div></div></div></details></article>`;
		})
		.join("");
};
